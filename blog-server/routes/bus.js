...
router.post('/', async (req, res,next) => {
  try {
    ...
    if(modelName in RESOURCE_POST_MAP){
      body = RESOURCE_POST_MAP[modelName]['body'](body,req._id)//将body和所需要信息合并对象
    }
    const result = await req.Model.create(body)//相应创建记录
    if (modelName in POP_POST_MAP) {
      ...
      await _model[queryAct](refId,options(_id))//如果是article就是给对应分类加上aid
    }
    res.send(200, {
      message: '提交成功',
      data: {
